<template>
  <div id="header" class="header">
		<div class="logo">
			<svg version="1.0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250.000000 236.000000" preserveAspectRatio="xMidYMid meet">
...
31 39 64 0 30 -7 45 -30 67 -27 26 -39 30 -98 32 -48 2 -76 -1 -97 -13z"/>
			...
			<!-- <img class="no-svg" src="../assets/d10.png"/> -->
		</div>
		...
		<div class="navigation" v-bind:class="{'mobile_menu': mobile_menu}">
			...
			<ul class="nav nav-tabs" style="text-align:right">
				<li class="nav-item"> 
					<a class="nav-link" href="#" v-bind:class="{active: view_screen == 'District'}" v-on:click="view_screen = 'District'">District</a> 
				</li> 
				<li class="nav-item"> 
					<a class="nav-link" href="#" v-bind:class="{active: view_screen == 'Name'}" v-on:click="view_screen = 'Name'">Name</a> 
				</li> 
				<li class="nav-item"> 
					<a class="nav-link" href="#" v-bind:class="{active: view_screen == 'Book'}" v-on:click="view_screen = 'Book'">Book</a> 
				</li> 
				<li class="nav-item"> 
					<a class="nav-link" href="#" v-bind:class="{active: view_screen == 'Items'}" v-on:click="view_screen = 'Items'">Item</a> 
				</li> 
				<li class="nav-item"> 
					<a class="nav-link" href="#" v-bind:class="{active: view_screen == 'NPC'}" v-on:click="view_screen = 'NPC'">NPC</a> 
				</li>
				<li class="nav-item"> 
		...
		<div class="top-right">
			...
			<div class="btn btn-sm btn-dark-outine" v-on:click="dark_mode = !dark_mode">
				<i class="fa-moon" v-bind:class="{fas: dark_mode == true, far: dark_mode == false}"></i> Dark Mode
			</div>
		</div>
  </div>
</template>
...
export default {
  ...
  watch: {
    view_screen: function(newVal, oldVal) {
      window.eventBus.$emit("view_screen_change", newVal);
    },
    dark_mode: function(newVal, oldVal) {
			console.log();
      window.eventBus.$emit("view_mode_change", newVal);
    }
  }
};
</script>
